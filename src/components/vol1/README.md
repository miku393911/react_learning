# React 実践練習問題集 Vol.1 (全31問)

👉 **学習記録・解説・コード分析はこちら**: [LEARNING_LOG.md](../LEARNING_LOG.md)

学習した「ロジック・条件分岐」と「配列・コールバック」の内容をベースに、Reactの `useState` と `useEffect` を使って実装する練習問題（Vol.1）です。
...
コンポーネントが表示されたら、「読み込み中...」と表示し、2秒後（`setTimeout`利用）に「データ読み込み完了！」と表示を切り替えてください。
*   **Hooks:** `useState`, `useEffect`
...
ハートの絵文字を使ったお気に入りボタンを作成してください。ボタンをクリックするたびに、以下のように表示が切り替わるようにしてください：
*   お気に入りでない状態: 白抜きのハート「🤍 お気に入り」
*   お気に入り状態: 赤いハート「❤️ お気に入り」
*   **Hooks:** `useState` (真偽値で状態管理)